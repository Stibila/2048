...
GameManager.prototype.showPlayerSelector = function (playerList) {
  ...
  this.p0.innerHTML = '<a href="../en/">Anglický jazyk / English language</a>';
  this.p1.innerHTML = 'Táto hra umožňuje viacerým hráčom hrať z jedného prehliadača, so zachovaním štatistík, ale aj rozohratej hry pre každého zvlášť. Prosím, zachovajte pravidlo: <strong>jeden hráč na osobu</strong>. Ďakujem.';
  ...
  divAddPlayer.innerHTML = 'Nový hrač';
...
//  this.form.onSubmit = "alert('Teraz sa tento formular odosle asynchronne na server a spusti sa nova hra.'); this.hideEverything(); return false;"
  ...
  this.submit.innerHTML = 'Začať hru';
  ...
  this.p1.addEventListener('mouseover', function() {popup('Tvoje meno bude uložené len na tvojom počítači. Nebude odoslané na server.');}, false);
  this.p1.addEventListener('mouseout', function() {popdown();}, false);
  this.p2.addEventListener('mouseover', function() {popup('Tvoj vek bude zaznamenaný pre štatistické účely.');}, false);
  this.p2.addEventListener('mouseout', function() {popdown();}, false);
  this.p3.addEventListener('mouseover', function() {popup('Tvoje pohlavie bude zaznamenané pre štatistické účely.');}, false);
  this.p3.addEventListener('mouseout', function() {popdown();}, false);
  this.p4.addEventListener('mouseover', function() {popup('Povedz nám, koľko času stráviš približne hraním videohier týždenne.');}, false);
  this.p4.addEventListener('mouseout', function() {popdown();}, false);
  this.p5.addEventListener('mouseover', function() {popup('Vyber žánre hier, ktoré hráš najčastejšie a ktoré najviac obľubuješ.');}, false);
  ...
  this.p4.innerHTML = '<b>Koľko času týždenne stráviš hraním videohier:</b><br /><select name="weekly">' + 
                          '<option value="0">Nehrávam videohry</option>' +
                          '<option value="1">Menej než 2 hodiny</option>' +
                          '<option value="2">Medzi 2 až 10 hodín</option>' +
                          '<option value="3">Medzi 10 až 20 hodín</option>' +
                          '<option value="4">Medzi 20 až 40 hodín</option>' +
                          '<option value="5">Viac než 40 hodín</option> </select>';
  //4 kolko casu travia hranim
  //5ake hry hraju
  this.p5.innerHTML = '<b>Aké žánre hier obľubuješ:</b><br />' +
                          '<table style="margin-left:auto; margin-right:auto; text-align:left;">' +
                          '<tr><td><label><input type="checkbox" name="g0" value="1">Akčné</label></td>' +
                          '<td>    <label><input type="checkbox" name="g1" value="1">Strieľačky</label></td></tr>' +
                          '<tr><td><label><input type="checkbox" name="g2" value="1">Adventúry</label></td>' +
                          '<td>    <label><input type="checkbox" name="g3" value="1">Na hrdinov (RPG)</label></td></tr>' +
                          '<tr><td><label><input type="checkbox" name="g4" value="1">Simulátory</label></td>' +
                          '<td>    <label><input type="checkbox" name="g5" value="1">Stratégie</label></td></tr>' +
                          '<tr><td><label><input type="checkbox" name="g6" value="1">Športové</label></td>' +
                          '<td>    <label><input type="checkbox" name="g7" value="1">Logické</label></td></tr>' +
                          '</table>';

  this.p6.innerHTML = '<b>Tvoje skúsenosti s hrou 2048:</b><br /><select name="experience">' +
                          '<option value="0">[0] Nikdy som o tej hre nepočul/a.</option>' +
                          '<option value="1">[1] Poznám ju, ale nehral/a som ju.</option>' +
                          '<option value="2">[2] Nikdy som nedosiahol/la 2048.</option>' +
                          '<option value="3">[3] Niekoľko krát som dosiahol/la 2048.</option>' +
                          '<option value="4">[4] Dosiahol/la som 4096 alebo 8192.</option> </select>';
